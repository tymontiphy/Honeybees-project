...
//                             <button className="buy-button" onClick={() => console.log('Added to cart')}>Add to Cart</button>
...
const API_URL = 'http://localhost:5000/products';  // Adjust the backend URL
const ORDER_URL = 'http://localhost:5000/orders';  // Order API endpoint
...
const Home = () => {
    ...
    //     try {
    //         // Step 1: Create order (assuming user_id = 1 for now)
    //         const orderResponse = await axios.post(ORDER_URL, { user_id: 1 });
    //         const orderId = orderResponse.data.id;

    //         // Step 2: Add items from the cart to the order
